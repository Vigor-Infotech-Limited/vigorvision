...
from vigorvision.data.utils import polygons2masks, polygons2masks_overlap
from vigorvision.utils import LOGGER, colorstr
from vigorvision.utils.checks import check_version
from vigorvision.utils.instance import Instances
from vigorvision.utils.metrics import bbox_ioa
from vigorvision.utils.ops import segment2box, xywh2xyxy, xyxyxyxy2xywhr
from vigorvision.utils.torch_utils import TORCHVISION_0_10, TORCHVISION_0_11, TORCHVISION_0_13
...
class BaseTransform:
    """
    Base class for image transformations in the vigorvision library.
...
class Compose:
    ...
    def __init__(self, transforms):
        ...
        Examples:
            >>> from vigorvision.data.augment import Compose, RandomHSV, RandomFlip
...
class Mosaic(BaseMixTransform):
    ...
    Examples:
        >>> from vigorvision.data.augment import Mosaic
...
class MixUp(BaseMixTransform):
    ...
    Examples:
        >>> from vigorvision.data.augment import MixUp
        >>> dataset = YourDataset(...)  # Your image dataset
    ...
    def __init__(self, dataset, pre_transform=None, p=0.0) -> None:
        ...
        values and labels. This implementation is designed for use with the vigorvision Vision framework.
...
class CutMix(BaseMixTransform):
    ...
    Examples:
        >>> from vigorvision.data.augment import CutMix
        >>> dataset = YourDataset(...)  # Your image dataset
...
class RandomHSV:
    ...
    Examples:
        >>> import numpy as np
        >>> from vigorvision.data.augment import RandomHSV
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            # lut_hue = ((x * (r[0] + 1)) % 180).astype(dtype)   # original hue implementation from vigorvision<=8.3.78
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (dict): A dictionary containing the following keys:
                ...
                'instances' (vigorvision.utils.instance.Instances): An object containing bounding boxes and
                    optionally keypoints.

        Returns:
            (dict): The same dictionary with the flipped image and updated instances:
                'img' (numpy.ndarray): The flipped image.
                'instances' (vigorvision.utils.instance.Instances): Updated instances matching the flipped image.
...
class LetterBox:
    ...
    def __init__(self, new_shape=(640, 640), auto=False, scale_fill=False, scaleup=True, center=True, stride=32):
        ...
        Args:
            ...
            stride (int): Stride of the model (e.g., 32 for Visionv5).
...
class CopyPaste(BaseMixTransform):
    ...
    Examples:
        >>> from vigorvision.data.augment import CopyPaste
        >>> dataset = YourDataset(...)  # Your image dataset
...
class Albumentations:
    ...
    def __init__(self, p=1.0):
        """
        Initialize the Albumentations transform object for Vision bbox formatted parameters.
        ...
        try:
            ...
            self.transform = (
                A.Compose(T, bbox_params=A.BboxParams(format="Vision", label_fields=["class_labels"]))
...
class Format:
    ...
    def _format_img(self, img):
        """
        Formats an image for Vision from a Numpy array to a PyTorch tensor.
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    This function creates a composition of image augmentation techniques to prepare images for Vision training.
    ...
    Examples:
        >>> from vigorvision.data.dataset import VisionDataset
        >>> from vigorvision.utils import IterableSimpleNamespace
        >>> dataset = VisionDataset(img_path="path/to/images", imgsz=640)
...
def classify_transforms(
    ...
    """
    import torchvision.transforms as T  # scope for faster 'import vigorvision'
...
class ToTensor:
    ...
    def __init__(self, half=False):
        ...
        vigorvision Vision framework. It converts numpy arrays or PIL Images to PyTorch tensors, with an option
