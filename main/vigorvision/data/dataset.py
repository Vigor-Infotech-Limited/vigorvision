...
from vigorvision.utils import LOCAL_RANK, LOGGER, NUM_THREADS, TQDM, colorstr
from vigorvision.utils.instance import Instances
from vigorvision.utils.ops import resample_segments, segments2boxes
from vigorvision.utils.torch_utils import TORCHVISION_0_18
...
# vigorvision dataset *.cache version, >= 1.0.0 for vigorvision Vision models
DATASET_CACHE_VERSION = "1.0.3"


class VisionDataset(BaseDataset):
    """
    Dataset class for loading object detection and/or segmentation labels in Vision format.

    This class supports loading data for object detection, segmentation, pose estimation, and oriented bounding box
    (OBB) tasks using the Vision format.
    ...
    Methods:
        ...
        get_labels: Returns dictionary of labels for Vision training.
    ...
    Examples:
        >>> dataset = VisionDataset(img_path="path/to/images", data={"names": {0: "person"}}, task="detect")
        >>> dataset.get_labels()
    """
    ...
    def get_labels(self):
        """
        Returns dictionary of labels for Vision training.
...
class VisionMultiModalDataset(VisionDataset):
    """
    Dataset class for loading object detection and/or segmentation labels in Vision format with multi-modal support.

    This class extends VisionDataset to add text information for multi-modal model training, enabling models to
    ...
    Examples:
        >>> dataset = VisionMultiModalDataset(img_path="path/to/images", data={"names": {0: "person"}}, task="detect")
    ...
    def __init__(self, *args, data=None, task="detect", **kwargs):
        """
        Initialize a VisionMultiModalDataset.
    ...
    def build_transforms(self, hyp=None):
        ...
        if self.augment:
            ...
            # NOTE: this implementation is different from official Visione,
...
class GroundingDataset(VisionDataset):
    """
    Handles object detection tasks by loading annotations from a specified JSON file, supporting Vision format.

    This dataset is designed for grounding tasks where annotations are provided in a JSON file rather than
    the standard Vision format text files.
...
class VisionConcatDataset(ConcatDataset):
    """
    Dataset as a concatenation of multiple datasets.

    This class is useful to assemble different existing datasets for Vision training, ensuring they use the same
    collation function.

    Methods:
        collate_fn: Static method that collates data samples into batches using VisionDataset's collation function.

    Examples:
        >>> dataset1 = VisionDataset(...)
        >>> dataset2 = VisionDataset(...)
        >>> combined_dataset = VisionConcatDataset([dataset1, dataset2])
    """
    ...
    def collate_fn(batch):
        ...
        Returns:
            (dict): Collated batch with stacked tensors.
        """
        return VisionDataset.collate_fn(batch)
...
class ClassificationDataset:
    """
    Extends torchvision ImageFolder to support Vision classification tasks.
    ...
    def __init__(self, root, args, augment=False, prefix=""):
        """
        Initialize Vision object with root, image size, augmentations, and cache settings.
        ...
        """
        import torchvision  # scope for faster 'import vigorvision'
        ...
        if self.cache_ram:
            LOGGER.warning(
                ...
                "https://github.com/vigorvision/vigorvision/issues/9824, setting `cache_ram=False`."
            )
