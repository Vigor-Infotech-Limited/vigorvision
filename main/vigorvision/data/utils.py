...
from vigorvision.utils.checks import check_file, check_font, is_ascii
from vigorvision.utils.downloads import download, safe_download, unzip_file
from vigorvision.utils.ops import segments2boxes

HELP_URL = "See https://docs.vigorvision.com/datasets for dataset formatting guidance."
...
def check_file_speeds(files, threshold_ms=10, threshold_mb=50, max_files=5, prefix=""):
    ...
    if avg_ping < threshold_ms or avg_speed < threshold_mb:
        ...
        LOGGER.warning(
            ...
            f"See https://docs.vigorvision.com/guides/model-training-tips/"
        )
...
def visualize_image_annotations(image_path, txt_path, label_map):
    """
    Visualizes Vision annotations (bounding boxes and class labels) on an image.

    This function reads an image and its corresponding annotation file in Vision format, then
    ...
    Args:
        ...
        txt_path (str): The path to the annotation file in Vision format, that should contain one line per object.
    ...
    from vigorvision.utils.plotting import colors
...
def check_det_dataset(dataset, autodownload=True):
    ...
    for k in "train", "val":
        if k not in data:
            ...
            LOGGER.warning("renaming data YAML 'validation' key to 'val' to match Vision format.")
...
def check_cls_dataset(dataset, split=""):
    ...
    if not data_dir.is_dir():
        ...
        if str(dataset) == "imagenet":
            ...
            url = f"https://github.com/vigorvision/assets/releases/download/v0.0.0/{dataset}.zip"
    ...
    if not train_set.is_dir():
        ...
        if image_files:
            from vigorvision.data.split import split_classify_dataset
...
class HUBDatasetStats:
    ...
    Note:
        Download *.zip files from https://github.com/vigorvision/hub/tree/main/example_datasets
        i.e. https://github.com/vigorvision/hub/raw/main/example_datasets/coco8.zip for coco8.zip.

    Examples:
        >>> from vigorvision.data.utils import HUBDatasetStats
    ...
    def get_json(self, save=False, verbose=False):
        """Return dataset JSON for vigorvision HUB."""
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                from torchvision.datasets import ImageFolder  # scope for faster 'import vigorvision'
                ...
                dataset = VisionDataset(img_path=self.data[split], data=self.data, task=self.task)
                x = np.array(
                    [
    ...
    def process_images(self):
        """Compress images for vigorvision HUB."""
        from vigorvision.data import VisionDataset  # ClassificationDataset

        self.im_dir.mkdir(parents=True, exist_ok=True)  # makes dataset-hub/images/
        for split in "train", "val", "test":
            if self.data.get(split) is None:
                continue
            dataset = VisionDataset(img_path=self.data[split], data=self.data)
...
def compress_one_image(f, f_new=None, max_dim=1920, quality=50):
    ...
    Examples:
        >>> from pathlib import Path
        >>> from vigorvision.data.utils import compress_one_image
...
def load_dataset_cache_file(path):
    """Load an vigorvision *.cache dictionary from path."""
    import gc

    gc.disable()  # reduce pickle load time https://github.com/vigorvision/vigorvision/pull/1585
...
def save_dataset_cache_file(prefix, path, x, version):
    """Save an vigorvision dataset *.cache dictionary x to path."""
