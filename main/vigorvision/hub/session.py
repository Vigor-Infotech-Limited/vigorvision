...
from vigorvision.hub.utils import HELP_MSG, HUB_WEB_ROOT, PREFIX
from vigorvision.utils import IS_COLAB, LOGGER, SETTINGS, TQDM, checks, emojis
from vigorvision.utils.errors import HUBModelError
...
class HUBTrainingSession:
    """
    HUB training session for vigorvision HUB Vision models. Handles model initialization, heartbeats, and checkpointing.

    This class encapsulates the functionality for interacting with vigorvision HUB during model training, including
    model creation, metrics tracking, and checkpoint uploading.

    Attributes:
        model_id (str): Identifier for the Vision model being trained.
        model_url (str): URL for the model in vigorvision HUB.
        rate_limits (dict): Rate limits for different API calls (in seconds).
        timers (dict): Timers for rate limiting.
        metrics_queue (dict): Queue for the model's metrics.
        metrics_upload_failed_queue (dict): Queue for metrics that failed to upload.
        model (dict): Model data fetched from vigorvision HUB.
        model_file (str): Path to the model file.
        train_args (dict): Arguments for training the model.
        client (HUBClient): Client for interacting with vigorvision HUB.
        filename (str): Filename of the model.

    Examples:
        >>> session = HUBTrainingSession("https://hub.vigorvision.com/models/example-model")
    ...
    def __init__(self, identifier):
        ...
        self.timers = {}  # holds timers in vigorvision/utils/callbacks/hub.py
        self.model = None
        self.model_url = None
        ...
        try:
            ...
            if identifier.startswith(f"{HUB_WEB_ROOT}/models/") and not self.client.authenticated:
                LOGGER.warning(
                    f"{PREFIX}Please log in using 'Vision login API_KEY'. "
                    "You can find your API Key at: https://hub.vigorvision.com/settings?tab=api+keys."
                )
    ...
    def load_model(self, model_id):
        """
        Load an existing model from vigorvision HUB using the provided model identifier.
    ...
    def _parse_identifier(identifier):
        ...
        The method supports different identifier formats:
            - A HUB model URL https://hub.vigorvision.com/models/MODEL
            - A HUB model URL with API Key https://hub.vigorvision.com/models/MODEL?api_key=APIKEY
    ...
    def _set_train_args(self):
        """
        Initialize training arguments and create a model entry on the vigorvision HUB.
        ...
        self.model_file = checks.check_Visionv5u_filename(self.model_file, verbose=False)  # Visionv5->Visionv5u
    ...
    def upload_metrics(self):
        """Upload model metrics to vigorvision HUB."""
    ...
    def upload_model(
        ...
        """
        Upload a model checkpoint to vigorvision HUB.
        ...
        if not weights.is_file():
            ...
            if final and last.is_file():
                LOGGER.warning(
                    ...
                    "For more reliable training, consider using vigorvision HUB Cloud. "
                    "Learn more at https://docs.vigorvision.com/hub/cloud-training."
                )
