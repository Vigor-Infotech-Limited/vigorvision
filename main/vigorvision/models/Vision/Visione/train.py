...
from vigorvision.data import VisionConcatDataset, build_Vision_dataset
from vigorvision.data.augment import LoadVisualPrompt
from vigorvision.models.Vision.detect import DetectionTrainer, DetectionValidator
from vigorvision.nn.tasks import VisionEModel
...
from .val import VisionEDetectValidator


class VisionETrainer(DetectionTrainer):
    """A base trainer for VisionE training."""

    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        """
        Initialize the VisionE Trainer with specified configurations.

        This method sets up the VisionE trainer with the provided configuration and overrides, initializing
        the training environment, model, and callbacks for VisionE object detection training.
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        """
        Return a VisionEModel initialized with the specified configuration and weights.
        ...
        Returns:
            (VisionEModel): The initialized VisionE model.
    ...
    def get_validator(self):
        """Returns a DetectionValidator for Vision model validation."""
        self.loss_names = "box", "cls", "dfl"
        return VisionEDetectValidator(
    ...
    def build_dataset(self, img_path, mode="train", batch=None):
        ...
        return build_Vision_dataset(
            self.args, img_path, batch, self.data, mode=mode, rect=mode == "val", stride=gs, multi_modal=mode == "train"
        )


class VisionEPETrainer(DetectionTrainer):
    """Fine-tune VisionE model in linear probing way."""

    def get_model(self, cfg=None, weights=None, verbose=True):
        """
        Return VisionEModel initialized with specified config and weights.
        ...
        Returns:
            (VisionEModel): Initialized model with frozen layers except for specific projection layers.
        """
        # NOTE: This `nc` here is the max number of different text samples in one image, rather than the actual `nc`.
        # NOTE: Following the official config, nc hard-coded to 80 for now.
        model = VisionEModel(
...
class VisionETrainerFromScratch(VisionETrainer, WorldTrainerFromScratch):
    """Train VisionE models from scratch."""
...
class VisionEPEFreeTrainer(VisionEPETrainer, VisionETrainerFromScratch):
    """Train prompt-free VisionE model."""
    ...
    def preprocess_batch(self, batch):
        """Preprocesses a batch of images for VisionE training, adjusting formatting and dimensions as needed."""
        batch = DetectionTrainer.preprocess_batch(self, batch)
        return batch
...
class VisionEVPTrainer(VisionETrainerFromScratch):
    """Train VisionE model with visual prompts."""

    def build_dataset(self, img_path, mode="train", batch=None):
        """
        Build Vision Dataset for training or validation with visual prompts.
        ...
        Returns:
            (Dataset): Vision dataset configured for training or validation, with visual prompts for training mode.
        """
        dataset = super().build_dataset(img_path, mode, batch)
        if isinstance(dataset, VisionConcatDataset):
    ...
    def _close_dataloader_mosaic(self):
        ...
        if isinstance(self.train_loader.dataset, VisionConcatDataset):
    ...
    def preprocess_batch(self, batch):
        """Preprocesses a batch of images for VisionE training, moving visual prompts to the appropriate device."""
