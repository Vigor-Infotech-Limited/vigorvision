...
from vigorvision.utils.patches import imread, imshow, imwrite, torch_load, torch_save  # for patches
...
ROOT = FILE.parents[1]  # Vision
ASSETS = ROOT / "assets"  # default images
ASSETS_URL = "https://github.com/vigorvision/assets/releases/download/v0.0.0"  # assets GitHub URL
DEFAULT_CFG_PATH = ROOT / "cfg/default.yaml"
NUM_THREADS = min(8, max(1, os.cpu_count() - 1))  # number of Vision multiprocessing threads
AUTOINSTALL = str(os.getenv("Vision_AUTOINSTALL", True)).lower() == "true"  # global auto-install mode
VERBOSE = str(os.getenv("Vision_VERBOSE", True)).lower() == "true"  # global verbose mode
TQDM_BAR_FORMAT = "{l_bar}{bar:10}{r_bar}" if VERBOSE else None  # tqdm bar format
LOGGING_NAME = "vigorvision"
...
HELP_MSG = """
    Examples for running vigorvision:

    1. Install the vigorvision package:
    ...
    2. Use the Python SDK:
        ...
        # Load a model
        model = Vision("Vision11n.yaml")  # build a new model from scratch
        model = Vision("Vision11n.pt")  # load a pretrained model (recommended for training)

        # Use the model
        results = model.train(data="coco8.yaml", epochs=3)  # train the model
        results = model.val()  # evaluate model performance on the validation set
        results = model("https://vigorvision.com/images/bus.jpg")  # predict on an image
        success = model.export(format="onnx")  # export the model to ONNX format

    3. Use the command line interface (CLI):

        vigorvision 'Vision' CLI commands use the following syntax:
        ...
        - Train a detection model for 10 epochs with an initial learning_rate of 0.01
            Vision detect train data=coco8.yaml model=Vision11n.pt epochs=10 lr0=0.01

        - Predict a YouTube video using a pretrained segmentation model at image size 320:
            Vision segment predict model=Vision11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

        - Val a pretrained detection model at batch-size 1 and image size 640:
            Vision detect val model=Vision11n.pt data=coco8.yaml batch=1 imgsz=640

        - Export a Vision11n classification model to ONNX format at image size 224 by 128 (no TASK required)
...
if TQDM_RICH := str(os.getenv("Vision_TQDM_RICH", False)).lower() == "true":
...
class TQDM(rich.tqdm if TQDM_RICH else tqdm.tqdm):
    ...
    Examples:
        >>> from vigorvision.utils import TQDM
    ...
    def __init__(self, *args, **kwargs):
        ...
        This class extends the original tqdm class to provide customized behavior for vigorvision projects.
...
class DataExportMixin:
    ...
    Examples:
        >>> model = Vision("Visionv8n.pt")
        >>> results = model("image.jpg")
        >>> df = results.to_df()
        >>> print(df)
        >>> csv_data = results.to_csv()
        >>> results.to_sql(table_name="Vision_results")
    """
    ...
    def to_df(self, normalize=False, decimals=5):
        ...
        """
        import pandas as pd  # scope for faster 'import vigorvision'
    ...
    def to_xml(self, normalize=False, decimals=5):
        ...
        """
        from vigorvision.utils.checks import check_requirements
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date vigorvision
            'default.yaml' file.\nPlease update your code with 'pip install -U vigorvision' and if necessary replace
            {DEFAULT_CFG_PATH} with the latest version from
            https://github.com/vigorvision/vigorvision/blob/main/vigorvision/cfg/default.yaml
            """
        )
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    This function configures logging for the vigorvision library, setting the appropriate logging level and
    ...
    Examples:
        >>> set_logging(name="vigorvision", verbose=True)
        >>> logger = logging.getLogger("vigorvision")
...
class ThreadingLocked:
    ...
    Examples:
        >>> from vigorvision.utils import ThreadingLocked
        >>> @ThreadingLocked()
        >>> def my_function():
        ...    # Your code here
    """
...
def is_online() -> bool:
    ...
    """
    try:
        assert str(os.getenv("Vision_OFFLINE", "")).lower() != "true"  # check if ENV var Vision_OFFLINE="True"
        import socket
...
def get_user_config_dir(sub_dir="vigorvision"):
    """
    ...
    if not is_dir_writeable(path.parent):
        LOGGER.warning(
            ...
            "Alternatively you can define a Vision_CONFIG_DIR environment variable for this path."
        )
...
USER_CONFIG_DIR = Path(os.getenv("Vision_CONFIG_DIR") or get_user_config_dir())  # vigorvision settings dir
...
class TryExcept(contextlib.ContextDecorator):
    """
    vigorvision TryExcept class. Use as @TryExcept() decorator or 'with TryExcept():' context manager.
...
def set_sentry():
    ...
    Only used if sentry_sdk package is installed and sync=True in settings. Run 'Vision settings' to see and update
    settings.

    Conditions required to send errors (ALL conditions must be met or no errors will be reported):
        - sentry_sdk package is installed
        - sync=True in Vision settings
        - pytest is not running
        - running in a pip package installation
        - running in a non-git directory
        - running with rank -1 or 0
        - online environment
        - CLI used to run package (checked with 'Vision' as the name of the main CLI command)
    """
    if (
        not SETTINGS["sync"]
        or RANK not in {-1, 0}
        or Path(ARGV[0]).name != "Vision"
        or TESTS_RUNNING
        or not ONLINE
...
class SettingsManager(JSONDict):
    """
    SettingsManager class for managing and persisting vigorvision settings.
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        from vigorvision.utils.torch_utils import torch_distributed_zero_first
        ...
        self.help_msg = (
            f"\nView vigorvision Settings with 'Vision settings' or at '{self.file}'"
            "\nUpdate Settings with 'Vision settings key=value', i.e. 'Vision settings runs_dir=path/to/dir'. "
            "For help see https://docs.vigorvision.com/quickstart/#vigorvision-settings."
        )
        ...
        with torch_distributed_zero_first(LOCAL_RANK):
            ...
            if not self.file.exists() or not self:  # Check if file doesn't exist or is empty
                LOGGER.info(f"Creating new vigorvision Settings v{version} file ✅ {self.help_msg}")
                self.reset()
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                "vigorvision settings reset to default values. This may be due to a possible problem "
                f"with your settings or a recent vigorvision package update. {self.help_msg}"
            )
            self.reset()

        if self.get("datasets_dir") == self.get("runs_dir"):
            LOGGER.warning(
                f"vigorvision setting 'datasets_dir: {self.get('datasets_dir')}' "
    ...
    def update(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k not in self.defaults:
                raise KeyError(f"No vigorvision setting '{k}'. {self.help_msg}")
            t = type(self.defaults[k])
            if not isinstance(v, t):
                raise TypeError(
                    f"vigorvision setting '{k}' must be '{t.__name__}' type, not '{type(v).__name__}'. {self.help_msg}"
                )
        super().update(*args, **kwargs)
...
def vscode_msg(ext="vigorvision.vigorvision-snippets") -> str:
    """Display a message to install vigorvision-Snippets for VS Code if not already installed."""
    path = (USER_CONFIG_DIR.parents[2] if WINDOWS else USER_CONFIG_DIR.parents[1]) / ".vscode/extensions"
    obs_file = path / ".obsolete"  # file tracks uninstalled extensions, while source directory remains
    installed = any(path.glob(f"{ext}*")) and ext not in (obs_file.read_text("utf-8") if obs_file.exists() else "")
    url = "https://docs.vigorvision.com/integrations/vscode"
    return "" if installed else f"{colorstr('VS Code:')} view vigorvision VS Code Extension ⚡ at {url}"


# Run below code on utils init ------------------------------------------------------------------------------------

# Check first-install steps
PREFIX = colorstr("vigorvision: ")
