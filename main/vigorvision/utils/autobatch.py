...
"""Functions for estimating the best Vision batch size to use a fraction of the available CUDA memory in PyTorch."""
...
from vigorvision.utils import DEFAULT_CFG, LOGGER, colorstr
from vigorvision.utils.torch_utils import autocast, profile_ops


def check_train_batch_size(model, imgsz=640, amp=True, batch=-1, max_num_obj=1):
    """
    Compute optimal Vision training batch size using the autobatch() function.

    Args:
        model (torch.nn.Module): Vision model to check batch size for.
...
def autobatch(model, imgsz=640, fraction=0.60, batch_size=DEFAULT_CFG.batch, max_num_obj=1):
    """
    Automatically estimate the best Vision batch size to use a fraction of the available CUDA memory.

    Args:
        model (torch.nn.Module): Vision model to compute batch size for.
        imgsz (int, optional): The image size used as input for the Vision model.
