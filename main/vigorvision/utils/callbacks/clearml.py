...
from vigorvision.utils import LOGGER, SETTINGS, TESTS_RUNNING
...
def on_pretrain_routine_start(trainer) -> None:
    ...
    try:
        if task := Task.current_task():
            ...
            task = Task.init(
                project_name=trainer.args.project or "vigorvision",
            ...
            LOGGER.warning(
                ...
                "please add clearml-init and connect your arguments before initializing Vision."
            )
...
def on_train_epoch_end(trainer) -> None:
    """Logs debug samples for the first epoch of Vision training and reports current training progress."""
    if task := Task.current_task():
        # Log debug samples
        if trainer.epoch == 1:
