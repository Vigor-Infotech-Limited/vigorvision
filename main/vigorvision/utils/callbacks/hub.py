...
from vigorvision.hub import HUB_WEB_ROOT, PREFIX, HUBTrainingSession, events
from vigorvision.utils import LOGGER, RANK, SETTINGS


def on_pretrain_routine_start(trainer):
    """Create a remote vigorvision HUB session to log local model training."""
...
def on_fit_epoch_end(trainer):
    """Upload training progress metrics to vigorvision HUB at the end of each epoch."""
    if session := getattr(trainer, "hub_session", None):
...
def on_model_save(trainer):
    """Upload model checkpoints to vigorvision HUB with rate limiting."""
    if session := getattr(trainer, "hub_session", None):
...
def on_train_end(trainer):
    """Upload final model and metrics to vigorvision HUB at the end of training."""
    if session := getattr(trainer, "hub_session", None):
