...
from vigorvision.data.dataset import GroundingDataset, VisionDataset, VisionMultiModalDataset
from vigorvision.data.loaders import (
    LOADERS,
    LoadImagesAndVideos,
    LoadPilAndNumpy,
    LoadScreenshots,
...
from vigorvision.data.utils import IMG_FORMATS, PIN_MEMORY, VID_FORMATS
from vigorvision.utils import RANK, colorstr
from vigorvision.utils.checks import check_file
...
def build_Vision_dataset(cfg, img_path, batch, data, mode="train", rect=False, stride=32, multi_modal=False):
    """Build and return a Vision dataset based on configuration parameters."""
    dataset = VisionMultiModalDataset if multi_modal else VisionDataset
    return dataset(
        img_path=img_path,
        imgsz=cfg.imgsz,
        batch_size=batch,
...
def check_source(source):
    ...
    if isinstance(source, (str, int, Path)):  # int for local usb camera
        ...
        raise TypeError("Unsupported image type. For supported types see https://docs.vigorvision.com/modes/predict")
