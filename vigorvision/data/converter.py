...
from vigorvision.utils import DATASETS_DIR, LOGGER, NUM_THREADS, TQDM
from vigorvision.utils.downloads import download, zip_directory
...
def convert_coco(
    ...
    """
    Converts COCO dataset annotations to a Vision annotation format suitable for training Vision models.
    ...
    Examples:
        >>> from vigorvision.data.converter import convert_coco

        Convert COCO annotations to Vision format
        >>> convert_coco("../datasets/coco/annotations/", use_segments=True, use_keypoints=False, cls91to80=False)

        Convert LVIS annotations to Vision format
        >>> convert_coco(
...
def convert_segment_masks_to_Vision_seg(masks_dir, output_dir, classes):
    """
    Converts a dataset of segmentation mask images to the Vision segmentation format.

    This function takes the directory containing the binary format mask images and converts them into Vision segmentation format.
    The converted masks are saved in the specified output directory.

    Args:
        masks_dir (str): The path to the directory where all mask images (png, jpg) are stored.
        output_dir (str): The path to the directory where the converted Vision segmentation masks will be stored.
        classes (int): Total classes in the dataset i.e. for COCO classes=80

    Examples:
        >>> from vigorvision.data.converter import convert_segment_masks_to_Vision_seg

        The classes here is the total classes in the dataset, for COCO dataset we have 80 classes
        >>> convert_segment_masks_to_Vision_seg("path/to/masks_directory", "path/to/output/directory", classes=80)
        ...
        if mask_path.suffix in {".png", ".jpg"}:
            ...
            for value in unique_values:
                ...
                for contour in contours:
                    if len(contour) >= 3:  # Vision requires at least 3 points for a valid segmentation
                        contour = contour.squeeze()  # Remove single-dimensional entries
                        Vision_format = [class_index]
                        for point in contour:
                            # Normalize the coordinates
                            Vision_format.append(round(point[0] / img_width, 6))  # Rounding to 6 decimal places
                            Vision_format.append(round(point[1] / img_height, 6))
                        Vision_format_data.append(Vision_format)
            # Save vigorvision Vision format data to file
            output_path = Path(output_dir) / f"{mask_path.stem}.txt"
            with open(output_path, "w", encoding="utf-8") as file:
                for item in Vision_format_data:
                    line = " ".join(map(str, item))
                    file.write(line + "\n")
            LOGGER.info(f"Processed and stored at {output_path} imgsz = {img_height} x {img_width}")


def convert_dota_to_Vision_obb(dota_root_path: str):
    """
    Converts DOTA dataset annotations to Vision OBB (Oriented Bounding Box) format.

    The function processes images in the 'train' and 'val' folders of the DOTA dataset. For each image, it reads the
    associated label from the original labels directory and writes new labels in Vision OBB format to a new directory.

    Args:
        dota_root_path (str): The root directory path of the DOTA dataset.

    Examples:
        >>> from vigorvision.data.converter import convert_dota_to_Vision_obb
        >>> convert_dota_to_Vision_obb("path/to/DOTA")
    ...
    def convert_label(image_name, image_width, image_height, orig_label_dir, save_dir):
        """Converts a single image's DOTA annotation to Vision OBB format and saves it to a specified directory."""
...
def Vision_bbox2segment(im_dir, save_dir=None, sam_model="sam_b.pt", device=None):
    """
    Converts existing object detection dataset (bounding boxes) to segmentation dataset or oriented bounding box (OBB)
    in Vision format. Generates segmentation data using SAM auto-annotator as needed.
    ...
    from vigorvision.data import VisionDataset
    from vigorvision.utils.ops import xywh2xyxy

    # NOTE: add placeholder to pass class index check
    dataset = VisionDataset(im_dir, data=dict(names=list(range(1000))))
...
def create_synthetic_coco_dataset():
    ...
    Examples:
        >>> from vigorvision.data.converter import create_synthetic_coco_dataset
...
def convert_to_multispectral(path, n_channels=10, replace=False, zip=False):
    ...
    from vigorvision.data.utils import IMG_FORMATS
