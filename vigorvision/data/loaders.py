...
from vigorvision.data.utils import FORMATS_HELP_MSG, IMG_FORMATS, VID_FORMATS
from vigorvision.utils import IS_COLAB, IS_KAGGLE, LOGGER, ops
from vigorvision.utils.checks import check_requirements
from vigorvision.utils.patches import imread
...
class SourceTypes:
    ...
    making predictions with Vision models.
...
class LoadStreams:
    ...
    def __iter__(self):
        """Iterates through Vision image feed and re-opens unresponsive streams."""
        self.count = -1
        return self
...
class LoadScreenshots:
    """
    vigorvision screenshot dataloader for capturing and processing screen images.

    This class manages the loading of screenshot images for processing with Vision. It is suitable for use with
    `Vision predict source=screen`.
...
class LoadImagesAndVideos:
    """
    A class for loading and processing images and videos for Vision object detection.
    ...
    def __init__(self, path, batch=1, vid_stride=1, channels=3):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            a = str(Path(p).absolute())  # do not use .resolve() https://github.com/vigorvision/vigorvision/issues/2912
            if "*" in a:
...
def autocast_list(source):
    """Merges a list of sources into a list of numpy arrays or PIL images for vigorvision prediction."""
    files = []
    for im in source:
        if isinstance(im, (str, Path)):  # filename or uri
            ...
            raise TypeError(
                f"type {type(im).__name__} is not a supported vigorvision prediction source type. \n"
                f"See https://docs.vigorvision.com/modes/predict for supported source types."
            )
