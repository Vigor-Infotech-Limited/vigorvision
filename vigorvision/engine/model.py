...
from vigorvision.cfg import TASK2DATA, get_cfg, get_save_dir
from vigorvision.engine.results import Results
from vigorvision.nn.tasks import attempt_load_one_weight, guess_model_task, yaml_model_load
...
class Model(torch.nn.Module):
    """
    A base class for implementing Vision models, unifying APIs across different model types.

    This class provides a common interface for various operations related to Vision models, such as training,
    validation, prediction, exporting, and benchmarking. It handles different types of models, including those
    loaded from local files, vigorvision HUB, or Triton Server.
    ...
    Attributes:
        ...
        session (HUBTrainingSession): The vigorvision HUB session, if applicable.
    ...
    def __init__(
        self,
        model: Union[str, Path] = "Vision11n.pt",
        task: str = None,
        verbose: bool = False,
    ) -> None:
        """
        Initialize a new instance of the Vision model class.

        This constructor sets up the model based on the provided model path or name. It handles various types of
        model sources, including local files, vigorvision HUB models, and Triton Server models. The method
        ...
        Args:
            model (str | Path): Path or name of the model to load or create. Can be a local file path, a
                model name from vigorvision HUB, or a Triton Server model.
            task (str | None): The task type associated with the Vision model, specifying its application domain.
        ...
        # Check if vigorvision HUB model from https://hub.vigorvision.com
        if self.is_hub_model(model):
            from vigorvision.hub import HUBTrainingSession
        ...
        # Load or create new Vision model
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        Returns:
            (List[vigorvision.engine.results.Results]): A list of prediction results, each encapsulated in a
                Results object.

        Examples:
            >>> model = Vision("Vision11n.pt")
            >>> results = model("https://vigorvision.com/images/bus.jpg")
            >>> for r in results:
    ...
    def is_triton_model(model: str) -> bool:
        ...
        Examples:
            >>> Model.is_triton_model("http://localhost:8000/v2/models/Vision11n")
            True
            >>> Model.is_triton_model("Vision11n.pt")
            False
        """
    ...
    def is_hub_model(model: str) -> bool:
        """
        Check if the provided model is an vigorvision HUB model.

        This static method determines whether the given model string represents a valid vigorvision HUB model
        identifier.
        ...
        Returns:
            (bool): True if the model is a valid vigorvision HUB model, False otherwise.

        Examples:
            >>> Model.is_hub_model("https://hub.vigorvision.com/models/MODEL")
            True
            >>> Model.is_hub_model("Vision11n.pt")
            False
        """
        from vigorvision.hub import HUB_WEB_ROOT
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False) -> None:
        ...
        Examples:
            >>> model = Model()
            >>> model._new("Vision11n.yaml", task="detect", verbose=True)
        """
    ...
    def _load(self, weights: str, task=None) -> None:
        ...
        Examples:
            >>> model = Model()
            >>> model._load("Vision11n.pt")
            >>> model._load("path/to/weights.pth", task="detect")
        """
        if weights.lower().startswith(("https://", "http://", "rtsp://", "rtmp://", "tcp://")):
            weights = checks.check_file(weights, download_dir=SETTINGS["weights_dir"])  # download and return local file
        weights = checks.check_model_file_from_stem(weights)  # add suffix, i.e. Vision11n -> Vision11n.pt
    ...
    def _check_is_pytorch_model(self) -> None:
        ...
        if not (pt_module or pt_str):
            raise TypeError(
                ...
                f"i.e. 'Vision predict model=Vision11n.onnx'.\nTo run CUDA or MPS inference please pass the device "
    ...
    def load(self, weights: Union[str, Path] = "Vision11n.pt") -> "Model":
        """
    ...
    def save(self, filename: Union[str, Path] = "saved_model.pt") -> None:
        ...
        the date, vigorvision version, license information, and a link to the documentation.
    ...
    def embed(
        ...
        **kwargs: Any,
        ...
        Examples:
            >>> model = Vision("Vision11n.pt")
            >>> image = "https://vigorvision.com/images/bus.jpg"
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        """
        Performs predictions on the given image source using the Vision model.
        ...
        """
        if source is None:
            source = "https://vigorvision.com/images/boats.jpg" if self.task == "obb" else ASSETS
            LOGGER.warning(f"'source' is missing. Using 'source={source}'.")

        is_cli = (ARGV[0].endswith("Vision") or ARGV[0].endswith("vigorvision")) and any(
    ...
    def track(
        ...
        **kwargs: Any,
        ...
        Returns:
            (List[vigorvision.engine.results.Results]): A list of tracking results, each a Results object.
        ...
        if not hasattr(self.predictor, "trackers"):
            from vigorvision.trackers import register_tracker
    ...
    def val(
        ...
        **kwargs: Any,
        ...
        Args:
            validator (vigorvision.engine.validator.BaseValidator | None): An instance of a custom validator class for
                validating the model.
            **kwargs (Any): Arbitrary keyword arguments for customizing the validation process.

        Returns:
            (vigorvision.utils.metrics.DetMetrics): Validation metrics obtained from the validation process.
    ...
    def benchmark(
        ...
        **kwargs: Any,
        ...
        It uses the 'benchmark' function from the vigorvision.utils.benchmarks module. The benchmarking is
        ...
        """
        self._check_is_pytorch_model()
        from vigorvision.utils.benchmarks import benchmark
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        from a checkpoint, integrating with vigorvision HUB, and updating model and configuration after training.

        When using vigorvision HUB, if the session has a loaded model, the method prioritizes HUB training
        ...
        """
        self._check_is_pytorch_model()
        if hasattr(self.session, "model") and self.session.model.id:  # vigorvision HUB session with loaded model
            if any(kwargs):
    ...
    def tune(
        ...
        **kwargs: Any,
        ...
        When Ray Tune is enabled, it leverages the 'run_ray_tune' function from the vigorvision.utils.tuner module.
        ...
        self._check_is_pytorch_model()
        if use_ray:
            from vigorvision.utils.tuner import run_ray_tune
    ...
    def names(self) -> Dict[int, str]:
        ...
        using the 'check_class_names' function from the vigorvision.nn.autobackend module. If the predictor is not
    ...
    def clear_callback(self, event: str) -> None:
        ...
        Args:
            event (str): The name of the event for which to clear the callbacks. This should be a valid event name
                recognized by the vigorvision callback system.
        ...
        Notes:
            - This method affects both custom callbacks added by the user and default callbacks
              provided by the vigorvision framework.
    ...
    def task_map(self) -> dict:
        ...
        various tasks and modes within the vigorvision framework.
