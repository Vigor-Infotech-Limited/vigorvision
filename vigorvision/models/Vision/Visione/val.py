...
from vigorvision.data import VisionConcatDataset, build_dataloader, build_Vision_dataset
...
from vigorvision.nn.modules.head import VisionEDetect
from vigorvision.nn.tasks import VisionEModel
from vigorvision.utils import LOGGER, TQDM
from vigorvision.utils.torch_utils import select_device, smart_inference_mode


class VisionEDetectValidator(DetectionValidator):
    """
    A mixin class for VisionE model validation that handles both text and visual prompt embeddings.

    This mixin provides functionality to validate VisionE models using either text or visual prompt embeddings.
    ...
    def get_visual_pe(self, dataloader, model):
        ...
        using a VisionE model. It normalizes the embeddings and handles cases where no samples
        exist for a class.

        Args:
            dataloader (torch.utils.data.DataLoader): The dataloader providing training samples.
            model (VisionEModel): The VisionE model from which to extract visual prompt embeddings.

        Returns:
            (torch.Tensor): Visual prompt embeddings with shape (1, num_classes, embed_dim).
        """
        assert isinstance(model, VisionEModel)
    ...
    def __call__(self, trainer=None, model=None, refer_data=None, load_vp=False):
        ...
        Args:
            ...
            model (VisionEModel, optional): Model to validate. Required if `trainer` is not provided.
        ...
        if trainer is not None:
            ...
            elif isinstance(model.model[-1], VisionEDetect) and hasattr(model.model[-1], "lrpc"):  # prompt-free
...
class VisionESegValidator(VisionEDetectValidator, SegmentationValidator):
    """VisionE segmentation validator that supports both text and visual prompt embeddings."""
