...
from vigorvision.utils.tal import TORCH_1_10, dist2bbox, dist2rbox, make_anchors
from vigorvision.utils.torch_utils import fuse_conv_and_bn, smart_inference_mode
...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "VisionEDetect", "VisionESegment"


class Detect(nn.Module):
    """Vision Detect head for detection models."""
    ...
    def __init__(self, nc=80, ch=()):
        """Initialize the Vision detection layer with specified number of classes and channels."""
        super().__init__()
    ...
    def _inference(self, x):
        ...
        if self.export and self.format in {"tflite", "edgetpu"}:
            ...
            # See https://github.com/vigorvision/vigorvision/issues/7371
            grid_h = shape[2]
            grid_w = shape[3]
    ...
    def postprocess(preds: torch.Tensor, max_det: int, nc: int = 80):
        """
        Post-processes Vision model predictions.
...
class Segment(Detect):
    """Vision Segment head for segmentation models."""

    def __init__(self, nc=80, nm=32, npr=256, ch=()):
        """Initialize the Vision model attributes such as the number of masks, prototypes, and the convolution layers."""
...
class OBB(Detect):
    """Vision OBB detection head for detection with rotation models."""
...
class Pose(Detect):
    """Vision Pose head for keypoints models."""

    def __init__(self, nc=80, kpt_shape=(17, 3), ch=()):
        """Initialize Vision network with default parameters and Convolutional Layers."""
        super().__init__(nc, ch)
    ...
    def forward(self, x):
        """Perform forward pass through Vision model and return predictions."""
        bs = x[0].shape[0]  # batch size
...
class Classify(nn.Module):
    """Vision classification head, i.e. x(b,c1,20,20) to x(b,c2)."""

    export = False  # export mode

    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):
        """Initializes Vision classification head to transform input tensor from (b,c1,20,20) to (b,c2) shape."""
        super().__init__()
    ...
    def forward(self, x):
        """Performs a forward pass of the Vision model on input image data."""
...
class WorldDetect(Detect):
    """Head for integrating Vision detection models with semantic understanding from text embeddings."""

    def __init__(self, nc=80, embed=512, with_bn=False, ch=()):
        """Initialize Vision detection layer with nc classes and layer channels ch."""
        super().__init__(nc, ch)
...
class VisionESegment(VisionEDetect):
    """Vision segmentation head with text embedding capabilities."""

    def __init__(self, nc=80, nm=32, npr=256, embed=512, with_bn=False, ch=()):
        """Initialize VisionESegment with class count, mask parameters, and embedding dimensions."""
    ...
    def forward(self, x, text):
        ...
        if not has_lrpc:
            x = VisionEDetect.forward(self, x, text)
        else:
            x, mask = VisionEDetect.forward(self, x, text, return_mask=True)
...
class RTDETRDecoder(nn.Module):
    ...
    def forward(self, x, batch=None):
        ...
        """
        from vigorvision.models.utils.ops import get_cdn_group
