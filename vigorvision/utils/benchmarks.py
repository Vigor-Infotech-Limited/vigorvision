...
"""
Benchmark a Vision model formats for speed and accuracy.

Usage:
    from vigorvision.utils.benchmarks import ProfileModels, benchmark
    ProfileModels(['Vision11n.yaml', 'Visionv8s.yaml']).run()
    benchmark(model='Vision11n.pt', imgsz=160)
...
from vigorvision import Vision, VisionWorld
from vigorvision.cfg import TASK2DATA, TASK2METRIC
from vigorvision.engine.exporter import export_formats
from vigorvision.utils import ARM64, ASSETS, IS_JETSON, LINUX, LOGGER, MACOS, TQDM, WEIGHTS_DIR, YAML
from vigorvision.utils.checks import IS_PYTHON_3_13, check_imgsz, check_requirements, check_Vision, is_rockchip
from vigorvision.utils.downloads import safe_download
from vigorvision.utils.files import file_size
from vigorvision.utils.torch_utils import get_cpu_info, select_device


def benchmark(
    model=WEIGHTS_DIR / "Vision11n.pt",
    data=None,
    imgsz=160,
    half=False,
    int8=False,
    ...
    """
    Benchmark a Vision model across different formats for speed and accuracy.
    ...
    Examples:
        Benchmark a Vision model with default settings:
        >>> from vigorvision.utils.benchmarks import benchmark
        >>> benchmark(model="Vision11n.pt", imgsz=640)
    """
    ...
    for i, (name, format, suffix, cpu, gpu, _) in enumerate(zip(*export_formats().values())):
        ...
        try:
            ...
            if i in {6, 7, 8, 9, 10}:  # TF SavedModel, TF GraphDef, and TFLite, TF EdgeTPU and TF.js
                assert not isinstance(model, VisionWorld), "VisionWorldv2 TensorFlow exports not supported by onnx2tf yet"
                # assert not IS_PYTHON_MINIMUM_3_12, "TFLite exports not supported on Python>=3.12 yet"
            if i == 11:  # Paddle
                assert not isinstance(model, VisionWorld), "VisionWorldv2 Paddle exports not supported yet"
                assert model.task != "obb", "Paddle OBB bug https://github.com/PaddlePaddle/Paddle/issues/72024"
                assert not is_end2end, "End-to-end models not supported by PaddlePaddle yet"
                assert (LINUX and not IS_JETSON) or MACOS, "Windows and Jetson Paddle exports not supported yet"
            if i == 12:  # MNN
                assert not isinstance(model, VisionWorld), "VisionWorldv2 MNN exports not supported yet"
            if i == 13:  # NCNN
                assert not isinstance(model, VisionWorld), "VisionWorldv2 NCNN exports not supported yet"
            if i == 14:  # IMX
                assert not is_end2end
                assert not isinstance(model, VisionWorld), "VisionWorldv2 IMX exports not supported"
                assert model.task == "detect", "IMX only supported for detection task"
                assert "C2f" in model.__str__(), "IMX only supported for Visionv8"  # TODO: enable for Vision11
            if i == 15:  # RKNN
                assert not isinstance(model, VisionWorld), "VisionWorldv2 RKNN exports not supported yet"
            ...
            if format == "-":
                ...
                exported_model = Vision(filename, task=model.task)
    ...
    # Print results
    check_Vision(device=device)  # print system info
    ...
    if verbose and isinstance(verbose, float):
        ...
        floor = verbose  # minimum metric floor to pass, i.e. = 0.29 mAP for Visionv5n
...
class RF100Benchmark:
    """
    Benchmark Vision model performance across various formats for speed and accuracy.

    This class provides functionality to benchmark Vision models on the RF100 dataset collection.
    ...
    def __init__(self):
        """Initialize the RF100Benchmark class for benchmarking Vision model performance across various formats."""
        self.ds_names = []
        self.ds_cfg_list = []
        self.rf = None
    ...
    def parse_dataset(self, ds_link_txt="datasets_links.txt"):
        ...
        os.chdir("rf-100")
        os.mkdir("vigorvision-benchmarks")
        safe_download("https://github.com/vigorvision/assets/releases/download/v0.0.0/datasets_links.txt")
            ...
            for line in file:
                try:
                    ...
                    if not Path(proj_version).exists():
                        self.rf.workspace(workspace).project(project).version(version).download("Visionv8")
                    else:
...
class ProfileModels:
    ...
    Examples:
        ...
        >>> from vigorvision.utils.benchmarks import ProfileModels
        >>> profiler = ProfileModels(["Vision11n.yaml", "Visionv8s.yaml"], imgsz=640)
        >>> profiler.run()
    """
    ...
    def run(self):
        """
        Profile Vision models for speed and accuracy across various formats including ONNX and TensorRT.
        ...
        Examples:
            Profile models and print results
            >>> from vigorvision.utils.benchmarks import ProfileModels
            >>> profiler = ProfileModels(["Vision11n.yaml", "Visionv8s.yaml"])
    ...
    def profile_tensorrt_model(self, engine_file: str, eps: float = 1e-3):
        """
        Profile Vision model performance with TensorRT, measuring average run time and standard deviation.
