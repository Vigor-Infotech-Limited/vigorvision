...
from vigorvision.utils import LOGGER, RANK, SETTINGS, TESTS_RUNNING, ops
from vigorvision.utils.metrics import ClassifyMetrics, DetMetrics, OBBMetrics, PoseMetrics, SegmentMetrics
...
try:
    ...
    # Names of plots created by vigorvision that are logged to Comet
...
def _get_comet_model_name() -> str:
    """Returns the model name for Comet from the environment variable COMET_MODEL_NAME or defaults to 'vigorvision'."""
    return os.getenv("COMET_MODEL_NAME", "vigorvision")
...
def _resume_or_create_experiment(args: SimpleNamespace) -> None:
    ...
    try:
        ...
        experiment.log_other("Created from", "vigorvision")
...
def _fetch_trainer_metadata(trainer) -> dict:
    """Returns metadata for Vision training including epoch and asset saving status."""
...
def _scale_bounding_box_to_original_image_shape(
    ...
    """
    Vision resizes images during training and the label values are normalized based on this resized shape.
...
def _format_prediction_annotations(image_path, metadata, class_label_map=None, class_map=None) -> Optional[dict]:
    """Format Vision predictions for object detection visualization."""
...
def _log_plots(experiment, trainer) -> None:
    ...
    Args:
        ...
        trainer (vigorvision.engine.trainer.BaseTrainer): The trainer object containing validation metrics and save
            directory information.

    Examples:
        >>> from vigorvision.utils.callbacks.comet import _log_plots
...
def on_pretrain_routine_start(trainer) -> None:
    """Creates or resumes a CometML experiment at the start of a Vision pre-training routine."""
...
def on_fit_epoch_end(trainer) -> None:
    ...
    Args:
        trainer (BaseTrainer): The Vision trainer object containing training state, metrics, and configuration.
    ...
    if curr_epoch == 1:
        from vigorvision.utils.torch_utils import model_info_for_loggers
