...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Examples:
        >>> from vigorvision.utils.checks import parse_requirements
        >>> parse_requirements(package="vigorvision")
    """
    if package:
...
def check_imgsz(imgsz, stride=32, min_dim=1, max_dim=2, floor=0):
    ...
    if len(imgsz) > max_dim:
        msg = (
            ...
            "or an integer, i.e. 'Vision export imgsz=640,480' or 'Vision export imgsz=640'"
        )
        if max_dim != 1:
...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='vigorvision'
        try:
...
def check_latest_pypi_version(package_name="vigorvision"):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the vigorvision package is available on PyPI.
    ...
    if ONLINE and IS_PIP_PACKAGE:
        try:
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    f"New https://pypi.org/project/vigorvision/{latest} available 😃 "
                    f"Update with 'pip install -U vigorvision'"
                )
                return True
        except Exception:
            pass
...
def check_font(font="Arial.ttf"):
    ...
    """
    from matplotlib import font_manager  # scope for faster 'import vigorvision'
    ...
    url = f"https://github.com/vigorvision/assets/releases/download/v0.0.0/{name}"
...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds=""):
    """
    Check if installed dependencies meet vigorvision Vision models requirements and attempt to auto-update if needed.
    ...
    Examples:
        ...
        >>> check_requirements("vigorvision>=8.0.0")

        Check multiple packages
        >>> check_requirements(["numpy", "vigorvision>=8.0.0"])
    """
    ...
    if s:
        if install and AUTOINSTALL:  # check environment variable
            ...
            LOGGER.info(f"{prefix} vigorvision requirement{'s' * (n > 1)} {pkgs} not found, attempting AutoUpdate...")
            try:
                t = time.time()
...
def check_suffix(file="Vision11n.pt", suffix=".pt", msg=""):
    """
...
def check_Visionv5u_filename(file: str, verbose: bool = True):
    """
    Replace legacy Visionv5 filenames with updated Visionv5u filenames.
    ...
    """
    if "Visionv3" in file or "Visionv5" in file:
        if "u.yaml" in file:
            file = file.replace("u.yaml", ".yaml")  # i.e. Visionv5nu.yaml -> Visionv5n.yaml
        elif ".pt" in file and "u" not in file:
            original_file = file
            file = re.sub(r"(.*Visionv5([nsmlx]))\.pt", "\\1u.pt", file)  # i.e. Visionv5n.pt -> Visionv5nu.pt
            file = re.sub(r"(.*Visionv5([nsmlx])6)\.pt", "\\1u.pt", file)  # i.e. Visionv5n6.pt -> Visionv5n6u.pt
            file = re.sub(r"(.*Visionv3(|-tiny|-spp))\.pt", "\\1u.pt", file)  # i.e. Visionv3-spp.pt -> Visionv3-sppu.pt
            if file != original_file and verbose:
                LOGGER.info(
                    f"PRO TIP 💡 Replace 'model={original_file}' with new 'model={file}'.\nVisionv5 'u' models are "
                    f"trained with https://github.com/vigorvision/vigorvision and feature improved performance vs "
                    f"standard Visionv5 models trained with https://github.com/vigorvision/Visionv5.\n"
                )
    return file


def check_model_file_from_stem(model="Vision11n"):
    """
    ...
    if not path.suffix and path.stem in downloads.GITHUB_ASSETS_STEMS:
        return path.with_suffix(".pt")  # add suffix, i.e. Vision11n -> Vision11n.pt
    return model
...
def check_file(file, suffix="", download=True, download_dir=".", hard=True):
    ...
    file = check_Visionv5u_filename(file)  # Visionv5n -> Visionv5nu
    if (
        not file
...
def check_Vision(verbose=True, device=""):
    """
    Return a human-readable Vision software and hardware summary.

    Args:
        verbose (bool): Whether to print verbose information.
        device (str | torch.device): Device to use for Vision.
    """
    import psutil

    from vigorvision.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from vigorvision.utils import ENVIRONMENT  # scope to avoid circular import
    from vigorvision.utils.torch_utils import get_cpu_info, get_gpu_info
    ...
    package_info = {}
    for r in parse_requirements(package="vigorvision"):
        try:
...
def check_amp(model):
    """
    Checks the PyTorch Automatic Mixed Precision (AMP) functionality of a Vision11 model.

    If the checks fail, it means there are anomalies with AMP on the system that may cause NaN losses or zero-mAP
    results, so AMP will be disabled during training.

    Args:
        model (nn.Module): A Vision11 model instance.

    Returns:
        (bool): Returns True if the AMP functionality works correctly with Vision11 model, else False.

    Examples:
        >>> from vigorvision import Vision
        >>> from vigorvision.utils.checks import check_amp
        >>> model = Vision("Vision11n.pt").model.cuda()
        >>> check_amp(model)
    """
    from vigorvision.utils.torch_utils import autocast
    ...
    try:
        ...
        assert amp_allclose(Vision("Vision11n.pt"), im)
        LOGGER.info(f"{prefix}checks passed ✅")
    except ConnectionError:
        LOGGER.warning(f"{prefix}checks skipped. Offline and unable to download Vision11n for AMP checks. {warning_msg}")
    except (AttributeError, ModuleNotFoundError):
        LOGGER.warning(
            f"{prefix}checks skipped. "
            f"Unable to load Vision11n for AMP checks due to possible vigorvision package modifications. {warning_msg}"
        )
...
def print_args(args: Optional[dict] = None, show_file=True, show_func=False):
    ...
    def strip_auth(v):
        """Clean longer vigorvision HUB URLs by stripping potential authentication information."""
