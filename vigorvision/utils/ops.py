...
from vigorvision.utils.metrics import batch_probiou


class Profile(contextlib.ContextDecorator):
    """
    Visionv8 Profile class. Use as a decorator with @Profile() or as a context manager with 'with Profile():'.
    ...
    Examples:
        >>> from vigorvision.utils.ops import Profile
...
def segment2box(segment, width=640, height=640):
    ...
    # any 3 out of 4 sides are outside the image, clip coordinates first, https://github.com/vigorvision/vigorvision/pull/18294
...
def scale_boxes(img1_shape, boxes, img0_shape, ratio_pad=None, padding=True, xywh=False):
    ...
    Args:
        ...
        padding (bool): If True, assuming the boxes is based on image augmented by Vision style. If False then do regular
            rescaling.
...
def non_max_suppression(
    ...
    Args:
        prediction (torch.Tensor): A tensor of shape (batch_size, num_classes + 4 + num_masks, num_boxes)
            ...
            output by a model, such as Vision.
    ...
    if isinstance(prediction, (list, tuple)):  # Visionv8 model in validation model, output = (inference_out, loss_out)
...
def masks2segments(masks, strategy="all"):
    ...
    """
    from vigorvision.data.converter import merge_multi_segment
